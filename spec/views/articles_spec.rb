...
RSpec.describe "articles/show" do
  ...
  it "shows a note about the canonical URL after edit" do
    allow(article1).to receive_messages(canonical_url: "https://example.com/lamas", published_at: Time.zone.at(0),
  ...
  it "shows the original publication time for crossposts" do
    allow(article1).to receive_messages(canonical_url: "https://example.com/lamas", crossposted_at: Time.current,
                                        originally_published_at: Time.zone.at(0))
    render
    ...
    expect(rendered).to have_no_text("</time>")
  end
end
