...
RSpec.describe "Visiting article comments", :js do
  ...
  context "when all comments" do
    ...
    it "displays comments" do
      expect(page).to have_css(".single-comment-node", visible: :visible, count: 8)
    end

    it "displays child comments" do
      expect(page).to have_css(".comment--deep-1", visible: :visible, count: 3)
    end

    it "displays grandchild comments" do
      expect(page).to have_css(".comment--deep-2#comment-node-#{grandchild_comment.id}", visible: :visible,
                                                                                         count: 1)
    end
  end
  ...
  context "when root is specified" do
    ...
    it "displays child comments" do
      expect(page).to have_css(".comment--deep-1", visible: :visible, count: 2)
    end
