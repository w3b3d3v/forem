...
RSpec.describe "User discussion locks", :js do
  ...
  context "when an Article does not have a discussion lock" do
    ...
    it "doesn't show a DiscussionLock on the Article" do
      visit article.path
      expect(page).to have_no_css("#discussion-lock")
    end
    ...
    it "doesn't hide reply button on each comment on the Article page" do
      visit article.path
      expect(page).to have_css("[data-testid='reply-button-#{comment_one.id}']")
      expect(page).to have_css("[data-testid='reply-button-#{comment_two.id}']")
    end
  ...
  context "when an Article has a discussion lock" do
    ...
    it "shows a DiscussionLock on the Article" do
      visit article.path
      expect(page).to have_css("#discussion-lock")
    ...
    it "hides reply button on each comment on the Article page" do
      visit article.path
      expect(page).to have_no_css("[data-testid='reply-button-#{comment_one.id}']")
      expect(page).to have_no_css("[data-testid='reply-button-#{comment_two.id}']")
    end
